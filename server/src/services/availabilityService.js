...
export async function findAvailability({ date, time, people }) {
...
export async function findAvailabilitySuggestions({ date, time, people }) {
  ...
  const slots = await Promise.all(offsets.map(async off => {
    ...
    return findAvailability({ date, time: hhmm, people });
  }));
